...
psa_status_t generate_pox_report(uint8_t *token_buf, size_t token_size, uintptr_t faddr, int execution_output, uint8_t *cbor_report, size_t *cbor_report_len)
{
    ...
    attest_token_encode_add_bstr(&pox_ctx, LABEL_ATTESTATION_TOKEN, &token_buf_c);
    ...
    attest_token_encode_add_integer(&pox_ctx, LABEL_EXECUTABLE_ADDR, (int64_t)faddr);
    ...
    attest_token_encode_add_integer(&pox_ctx, LABEL_EXECUTION_RESULT, (int64_t)execution_output);
